...
async function findQuesy(searchArray){
  ...
  if (searchArray.length>=8){
    ...
    var query = { 
      $text: { 
        $search: `\"${searchArray[0]}\" \"${searchArray[1]}\" -mkv -mp4 -avi` 
      }
    }
  } else if (searchArray.length==3){
    var query = { 
      $text: { 
        $search: `\"${searchArray[0]}\" \"${searchArray[1]}\" \"${searchArray[2]}\" -mkv -mp4 -avi` 
      }
    }
  } else if (searchArray.length==4){
    var query = { 
      $text: { 
        $search: `\"${searchArray[0]}\" \"${searchArray[1]}\" \"${searchArray[2]}\" \"${searchArray[3]}\" -mkv -mp4 -avi` 
      }
    }
  } else if (searchArray.length==5){
    var query = { 
      $text: { 
        $search: `\"${searchArray[0]}\" \"${searchArray[1]}\" \"${searchArray[2]}\" \"${searchArray[3]}\" \"${searchArray[4]}\" -mkv -mp4 -avi` 
      }
    }
  } else if (searchArray.length==6){
    var query = { 
      $text: { 
        $search: `\"${searchArray[0]}\" \"${searchArray[1]}\" \"${searchArray[2]}\" \"${searchArray[3]}\" \"${searchArray[4]}\" \"${searchArray[5]}\" -mkv -mp4 -avi` 
      }
    }
  } else if (searchArray.length==7){
    var query = { 
      $text: { 
        $search: `\"${searchArray[0]}\" \"${searchArray[1]}\" \"${searchArray[2]}\" \"${searchArray[3]}\" \"${searchArray[4]}\" \"${searchArray[5]}\" \"${searchArray[6]}\" -mkv -mp4 -avi` 
      }
    }
  } else{var query ="";}
  return query;
